class Learning::Batch::OpSessionDecorator < SimpleDelegator
  ...
  def display_checkin_button
    if state == 'confirm' && check_in_time.blank?
      if Time.now < (start_datetime - 15.mins)
        ...
        "<div class='tooltip-span' data-toggle='tooltip' data-html='true' title='Chưa tới giờ điểm danh'><button type='button' class='btn btn-default btn-embossed'  disabled><span>#{ image_tag(ActionController::Base.helpers.asset_path('hotel-2.png')) }</span>Check-in</button></div>"
      end
    else
      if check_in_state == 'none'
        "<div class='tooltip-span' data-toggle='tooltip' data-html='true' title='Quên điểm danh'><button type='button' class='btn btn-danger'  disabled><span>#{ image_tag(ActionController::Base.helpers.asset_path('hotel-2.png')) }</span>Check-in</button></div>"
      elsif check_in_state == 'good'
        "<div class='tooltip-span' data-toggle='tooltip' data-html='true' title='Điểm danh đúng giờ'><button type='button' class='btn btn-success'  disabled><span>#{ image_tag(ActionController::Base.helpers.asset_path('hotel-2.png')) }</span>Check-in</button></div>"
      elsif check_in_state == 'late'
        "<div class='tooltip-span' data-toggle='tooltip' data-html='true' title='Điểm danh muộn'><button type='button' class='btn btn-warning' disabled><span>#{ image_tag(ActionController::Base.helpers.asset_path('hotel-2.png')) }</span>Check-in</button></div>"
      else
